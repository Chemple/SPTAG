...
template <typename T> bool GpuIndex<T>::CheckHeadIndexType() {
...
template <typename T>
ErrorCode GpuIndex<T>::LoadConfig(Helper::IniReader &p_reader) {
  IndexAlgoType algoType =
      p_reader.GetParameter("Base", "IndexAlgoType", IndexAlgoType::Undefined);
...
template <typename T>
ErrorCode GpuIndex<T>::LoadIndexDataFromMemory(
  ...
  if (m_pQuantizer) {
    m_extraSearcher.reset(new GpuExtraFullGraphSearcher<std::uint8_t>());
  } else {
    m_extraSearcher.reset(new GpuExtraFullGraphSearcher<T>());
  }
...
template <typename T>
ErrorCode GpuIndex<T>::LoadIndexData(
...
template <typename T>
ErrorCode GpuIndex<T>::SaveConfig(std::shared_ptr<Helper::DiskIO> p_configOut) {
...
#undef DefineBuildHeadParameter
  ...
  Helper::Convert::ConvertStringTo<int>(
      m_index->GetParameter("HashTableExponent").c_str(), m_options.m_hashExp);
...
template <typename T>
ErrorCode GpuIndex<T>::SaveIndexData(
    const std::vector<std::shared_ptr<Helper::DiskIO>> &p_indexStreams) {
...
template <typename T>
ErrorCode GpuIndex<T>::SearchIndex(QueryResult &p_query,
...
template <typename T>
ErrorCode GpuIndex<T>::SearchDiskIndex(QueryResult &p_query,
  ...
  if (!workSpace) {
    workSpace.reset(new ExtraWorkSpace());
#ifdef DEBUG
    workSpace->InitializeMaxResource(m_options.m_maxCheck, m_options.m_hashExp,
                                     m_options.m_searchInternalResultNum,
                                     max(m_options.m_postingPageLimit,
                                         m_options.m_searchPostingPageLimit + 1)
                                         << PageSizeEx,
  ...
  // NOTE(shiwen):bugs.
  auto query = p_queryResults->GetTarget();
  workSpace->SetQuery((char *)query);
  ...
  m_extraSearcher->SearchIndex_DEBUG(workSpace.get(), *p_queryResults, m_index,
                                     p_stats);
  m_workSpaceFactory->ReturnWorkSpace(std::move(workSpace));
  p_queryResults->SortResult();
  return ErrorCode::Success;
...
template <typename T>
ErrorCode GpuIndex<T>::DebugSearchDiskIndex(
  ...
  for (SizeType p = 0; p < partitions; p++) {
    ...
    m_extraSearcher->SearchIndex(workSpace.get(), newResults, m_index, p_stats,
                                 truth, found);
  }
  m_workSpaceFactory->ReturnWorkSpace(std::move(workSpace));
...
template <typename T>
void GpuIndex<T>::SelectHeadAdjustOptions(int p_vectorCount) {
  ...
  if (headCnt == 0) {
    ...
    SPTAGLIB_LOG(Helper::LogLevel::LL_Info,
                 "Setting requires to select none vectors as head, adjusted it "
                 "to %d vectors\n",
                 headCnt);
  }
  ...
  if (m_options.m_iBKTKmeansK > headCnt) {
    ...
    SPTAGLIB_LOG(
        ...
        "Setting of cluster number is less than head count, adjust it to %d\n",
        headCnt);
  }
  ...
  if (m_options.m_splitThreshold == 0) {
    m_options.m_splitThreshold = min(
        p_vectorCount - 1, static_cast<int>(m_options.m_selectThreshold * 2));
...
template <typename T>
int GpuIndex<T>::SelectHeadDynamicallyInternal(
...
template <typename T>
void GpuIndex<T>::SelectHeadDynamically(
  ...
  for (int select = 2; select <= m_options.m_selectThreshold; ++select) {
    ...
    while (l < r - 1) {
      ...
      double diff = static_cast<double>(p_selected.size()) / p_vectorCount -
                    m_options.m_ratio;
...
bool GpuIndex<T>::SelectHeadInternal(
  ...
  if (data.R() == 1) {
    ...
    SPTAGLIB_LOG(
        Helper::LogLevel::LL_Info,
        "Invoking BuildTrees used time: %.2lf minutes (about %.2lf hours).\n",
        elapsedSeconds / 60.0, elapsedSeconds / 3600.0);

    if (m_options.m_saveBKT) {
      std::stringstream bktFileNameBuilder;
      bktFileNameBuilder << m_options.m_vectorPath << ".bkt."
                         << m_options.m_iBKTKmeansK << "_"
                         << m_options.m_iBKTLeafSize << "_"
                         << m_options.m_iTreeNumber << "_"
                         << m_options.m_iSamples << "_"
                         << static_cast<int>(m_options.m_distCalcMethod)
                         << ".bin";
  ...
  if (!m_options.m_noOutput) {
    ...
    if (output == nullptr || outputIDs == nullptr ||
        ...
        !outputIDs->Initialize(
            (m_options.m_indexDirectory + FolderSep + m_options.m_headIDFile)
                .c_str(),
            std::ios::binary | std::ios::out)) {
      SPTAGLIB_LOG(
          Helper::LogLevel::LL_Error, "Failed to create output file:%s %s\n",
    ...
    for (int i = 0; i < selected.size(); i++) {
      ...
      if (outputIDs->WriteBinary(sizeof(vid), reinterpret_cast<char *>(&vid)) !=
          sizeof(vid)) {
        SPTAGLIB_LOG(Helper::LogLevel::LL_Error,
                     "Failed to write output file!\n");
        return false;
...
template <typename T>
ErrorCode GpuIndex<T>::BuildIndexInternal(
    std::shared_ptr<Helper::VectorSetReader> &p_reader) {
  ...
  if (m_options.m_buildHead) {
    ...
    }
    {
      ...
      if (m_index->BuildIndex(headvectorset, nullptr, false, true, true) !=
          ErrorCode::Success) {
        SPTAGLIB_LOG(Helper::LogLevel::LL_Error,
    ...
    if (LoadIndex(m_options.m_indexDirectory + FolderSep +
      ...
      SPTAGLIB_LOG(
          Helper::LogLevel::LL_Error, "Cannot load head index from %s!\n",
  ...
  if (m_options.m_enableSSD) {
    ...
    if (m_index == nullptr && LoadIndex(m_options.m_indexDirectory + FolderSep +
                                            m_options.m_headIndexFolder,
                                        m_index) != ErrorCode::Success) {
      SPTAGLIB_LOG(
          Helper::LogLevel::LL_Error, "Cannot load head index from %s!\n",
    ...
    if (m_extraSearcher != nullptr) {
      ...
      if (ptr == nullptr ||
        ...
        SPTAGLIB_LOG(
            Helper::LogLevel::LL_Error, "Failed to open headIDFile file:%s\n",
            (m_options.m_indexDirectory + FolderSep + m_options.m_headIDFile)
                .c_str());
        return ErrorCode::Fail;
      }
...
template <typename T> ErrorCode GpuIndex<T>::BuildIndex(bool p_normalized) {
  SPTAG::VectorValueType valueType =
      m_pQuantizer ? SPTAG::VectorValueType::UInt8 : m_options.m_valueType;
  SizeType dim =
      m_pQuantizer ? m_pQuantizer->GetNumSubvectors() : m_options.m_dim;
  std::shared_ptr<Helper::ReaderOptions> vectorOptions(
      new Helper::ReaderOptions(valueType, dim, m_options.m_vectorType,
  ...
  if (m_options.m_vectorPath.empty()) {
    ...
    m_options.m_vectorSize = 100 * 1024 * 1024;
  }
...
template <typename T>
ErrorCode GpuIndex<T>::BuildIndex(const void *p_data, SizeType p_vectorNum,
  ...
  if (p_shareOwnership) {
    vectorSet.reset(new BasicVectorSet(
        ByteArray((std::uint8_t *)p_data, sizeof(T) * p_vectorNum * p_dimension,
                  false),
    ...
    vectorSet.reset(new BasicVectorSet(arr, GetEnumValueType<T>(), p_dimension,
                                       p_vectorNum));
  }
  ...
  std::shared_ptr<Helper::VectorSetReader> vectorReader(
      new Helper::MemoryVectorReader(std::make_shared<Helper::ReaderOptions>(
                                         valueType, p_dimension,
...
template <typename T> ErrorCode GpuIndex<T>::UpdateIndex() {
  omp_set_num_threads(m_options.m_iSSDNumberOfThreads);
  m_index->SetParameter("NumberOfThreads",
                        std::to_string(m_options.m_iSSDNumberOfThreads));
  // m_index->SetParameter("MaxCheck", std::to_string(m_options.m_maxCheck));
...
template <typename T>
ErrorCode GpuIndex<T>::SetParameter(const char *p_param, const char *p_value,
...
template <typename T>
std::string GpuIndex<T>::GetParameter(const char *p_param,
...
GpuIndex<T>::GetIterator(const void *p_target, bool p_searchDeleted) const {};

template <typename T>
ErrorCode GpuIndex<T>::SearchIndexIterativeNext(QueryResult &p_query,
...
template <typename T>
ErrorCode GpuIndex<T>::SearchIndexIterativeEnd(
    std::unique_ptr<COMMON::WorkSpace> workSpace) const {};

template <typename T>
bool GpuIndex<T>::SearchIndexIterativeFromNeareast(QueryResult &p_query,
...
template <typename T>
std::unique_ptr<COMMON::WorkSpace> GpuIndex<T>::RentWorkSpace(int batch) const {
};

template <typename T>
ErrorCode GpuIndex<T>::SearchIndexWithFilter(
...
#define DefineVectorValueType(Name, Type)                                      \
  template class SPTAG::GpuSPANN::GpuIndex<Type>;
