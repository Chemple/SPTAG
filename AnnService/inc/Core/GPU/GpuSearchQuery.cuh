...
public:
  ...
  QueryResult()
      : m_target(nullptr), m_resultNum(0), m_withMeta(false),
        m_quantizedTarget(nullptr), m_quantizedSize(0) {}

  QueryResult(int p_resultNum) { Init(nullptr, p_resultNum, true); }
  ...
  QueryResult(const void *p_target, int p_resultNum, bool p_withMeta,
              BasicResult *p_results)
      : m_target(p_target), m_resultNum(p_resultNum), m_withMeta(p_withMeta),
        m_quantizedTarget((void *)p_target), m_quantizedSize(0) {
  ...
  QueryResult(const QueryResult &p_other) {
    ...
    if (m_resultNum > 0) {
      std::copy(p_other.m_results.Data(),
                p_other.m_results.Data() + m_resultNum, m_results.Data());
    }
    if (p_other.m_target != p_other.m_quantizedTarget) {
      ...
      std::copy(reinterpret_cast<std::uint8_t *>(p_other.m_quantizedTarget),
                reinterpret_cast<std::uint8_t *>(p_other.m_quantizedTarget) +
                    m_quantizedSize,
                reinterpret_cast<std::uint8_t *>(m_quantizedTarget));
    }
  }
  ...
  inline void SetResultNum(int p_resultNum) { m_resultNum = p_resultNum; }

  inline const void *GetTarget() { return m_target; }

  inline const void *GetQuantizedTarget() { return m_quantizedTarget; }
  ...
  inline bool HasQuantizedTarget() { return m_target != m_quantizedTarget; }
  ...
  inline BasicResult *GetResults() const { return m_results.Data(); }
  ...
  iterator begin() { return m_results.Data(); }

  iterator end() { return m_results.Data() + m_resultNum; }

  const_iterator begin() const { return m_results.Data(); }

  const_iterator end() const { return m_results.Data() + m_resultNum; }

protected:
  const void *m_target;

  const void *m_dev_query;
