...
private:
  void CreateCDict() {
    cdict = ZSTD_createCDict((void *)dictBuffer.data(), dictBuffer.size(),
                             compress_level);
    if (cdict == NULL) {
  ...
  std::string CompressWithDict(const std::string &src) {
    ...
    size_t compressed_size = ZSTD_compress_usingCDict(
        cctx, (void *)comp_buffer.data(), est_compress_size, src.data(),
        src.size(), cdict);
  ...
  std::size_t DecompressWithDict(const char *src, size_t srcSize, char *dst,
                                 size_t dstCapacity) {
  ...
  std::string CompressWithoutDict(const std::string &src) {
    ...
    size_t compressed_size =
        ZSTD_compress((void *)buffer.data(), est_comp_size, src.data(),
                      src.size(), compress_level);
    if (ZSTD_isError(compressed_size)) {
      SPTAGLIB_LOG(Helper::LogLevel::LL_Error, "ZSTD compress error %s, \n",
                   ZSTD_getErrorName(compressed_size));
      throw std::runtime_error("ZSTD compress error");
  ...
  std::size_t DecompressWithoutDict(const char *src, size_t srcSize, char *dst,
                                    size_t dstCapacity) {
    std::size_t const decomp_size =
        ZSTD_decompress((void *)dst, dstCapacity, src, srcSize);
    if (ZSTD_isError(decomp_size)) {
      SPTAGLIB_LOG(Helper::LogLevel::LL_Error, "ZSTD decompress error %s, \n",
                   ZSTD_getErrorName(decomp_size));
      throw std::runtime_error("ZSTD decompress failed.");
...
public:
  ...
  std::size_t TrainDict(const std::string &samplesBuffer,
                        const size_t *samplesSizes, unsigned nbSamples) {
    dictBuffer.resize(dictBufferCapacity);
    size_t dictSize = ZDICT_trainFromBuffer(
        (void *)dictBuffer.data(), dictBufferCapacity,
        (void *)samplesBuffer.data(), &samplesSizes[0], nbSamples);
  ...
  std::string GetDictBuffer() { return dictBuffer; }
  ...
  std::size_t Decompress(const char *src, size_t srcSize, char *dst,
                         size_t dstCapacity, const bool useDict) {
    return useDict ? DecompressWithDict(src, srcSize, dst, dstCapacity)
                   : DecompressWithoutDict(src, srcSize, dst, dstCapacity);
  }
