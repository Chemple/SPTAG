...
ErrorCode OutputResult(const std::string &p_output,
  ...
  if (!p_output.empty()) {
    auto ptr = f_createIO();
    if (ptr == nullptr ||
        !ptr->Initialize(p_output.c_str(), std::ios::binary | std::ios::out)) {
      SPTAGLIB_LOG(Helper::LogLevel::LL_Error, "Failed create file: %s\n",
                   p_output.c_str());
      return ErrorCode::FailedCreateFile;
    }
    int32_t i32Val = static_cast<int32_t>(p_results.size());
    if (ptr->WriteBinary(sizeof(i32Val), reinterpret_cast<char *>(&i32Val)) !=
        sizeof(i32Val)) {
    ...
    for (size_t i = 0; i < p_results.size(); ++i) {
      for (int j = 0; j < p_resultNum; ++j) {
        ...
        if (ptr->WriteBinary(sizeof(fVal), reinterpret_cast<char *>(&fVal)) !=
            sizeof(fVal)) {
          SPTAGLIB_LOG(Helper::LogLevel::LL_Error,
                       "Fail to write result file!\n");
          return ErrorCode::DiskIOFail;
...
template <typename ValueType>
void SearchSequential(GpuSPANN::GpuIndex<ValueType> *p_index, int p_numThreads,
                      std::vector<QueryResult> &p_results,
                      std::vector<GpuSPANN::SearchStats> &p_stats,
    ...
    threads.emplace_back([&, i]() {
      NumaStrategy ns =
          (p_index->GetDiskIndex() != nullptr)
              ...
              : NumaStrategy::LOCAL; // Only for SPANN, we need to avoid IO
                                     // threads overlap with search threads.
      ...
      while (true) {
        ...
        if (index < numQueries) {
          ...
          p_stats[index].m_totalLatency = p_stats[index].m_totalSearchLatency =
              exEndTime - startTime;
        } else {
          return;
        }
...
template <typename ValueType>
void Search(GpuSPANN::GpuIndex<ValueType> *p_index) {
  GpuSPANN::Options &p_opts = *(p_index->GetOptions());
  ...
  if (!p_opts.m_logFile.empty()) {
    SetLogger(std::make_shared<Helper::FileLogger>(Helper::LogLevel::LL_Info,
                                                   p_opts.m_logFile.c_str()));
  }
  ...
  if (!warmupFile.empty()) {
    ...
    std::vector<QueryResult> warmupResults(
        warmupNumQueries, QueryResult(NULL, max(K, internalResultNum), false));
    std::vector<GpuSPANN::SearchStats> warmpUpStats(warmupNumQueries);
    for (int i = 0; i < warmupNumQueries; ++i) {
      (*((COMMON::QueryResultSet<ValueType> *)&warmupResults[i]))
          .SetTarget(
              reinterpret_cast<ValueType *>(warmupQuerySet->GetVector(i)),
  ...
  std::vector<GpuSPANN::SearchStats> stats(numQueries);
  for (int i = 0; i < numQueries; ++i) {
  ...
  //   auto vectorReader =
  //   Helper::VectorSetReader::CreateInstance(vectorOptions); if
  //   (ErrorCode::Success == vectorReader->LoadFile(p_opts.m_vectorPath)) {
  ...
  if (!truthFile.empty()) {
    ...
    auto ptr = f_createIO();
    if (ptr == nullptr ||
        !ptr->Initialize(truthFile.c_str(), std::ios::in | std::ios::binary)) {
  ...
  SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "\nEx Elements Count:\n");
  PrintPercentiles<double, GpuSPANN::SearchStats>(
      stats,
      [](const GpuSPANN::SearchStats &ss) -> double {
  ...
  PrintPercentiles<double, GpuSPANN::SearchStats>(
      stats,
      [](const GpuSPANN::SearchStats &ss) -> double { return ss.m_exLatency; },
      "%.3lf");
  ...
  PrintPercentiles<int, GpuSPANN::SearchStats>(
      stats,
      [](const GpuSPANN::SearchStats &ss) -> int {
  ...
  PrintPercentiles<int, GpuSPANN::SearchStats>(
      stats,
      [](const GpuSPANN::SearchStats &ss) -> int { return ss.m_diskIOCount; },
      "%4d");
  ...
  if (p_opts.m_recall_analysis) {
    ...
    SizeType sampleK =
        headIndex->GetNumSamples() < 1000 ? headIndex->GetNumSamples() : 1000;
    float sampleE = 1e-6f;
...
#pragma omp parallel for schedule(dynamic)
    for (int i = 0; i < sampleSize; i++) {
      ...
      p_index->DebugSearchDiskIndex(queryBFHeads, internalResultNum, sampleK,
                                    nullptr, &truth[samples[i]], &tmpFound);
    ...
    SPTAGLIB_LOG(Helper::LogLevel::LL_Info,
                 "\tPercent of truths no shorter distance in query selected "
                 "heads: %f percent\n",
